...
RAILWAY_URL = 'https://gemini-to-word-railway-paywall-production.up.railway.app'
...
# { email: { 'active': True, 'plan': 'monthly', 'since': timestamp } }
paid_users = {}
...
def create_checkout():
    try:
        ...
        payload = {
            ...
            'return_url': f'{RAILWAY_URL}/success',
        }
...
def webhook():
    try:
        ...
        if event_type in ('payment.succeeded', 'subscription.active'):
            customer = event.get('data', {}).get('customer', {})
            email = customer.get('email', '').lower()
            if email:
                paid_users[email] = {
                    'active': True,
                    'plan': event.get('data', {}).get('product_id', 'unknown'),
                    'since': datetime.now().isoformat()
                }
                print(f'✅ Pro activated: {email}')

        if event_type in ('subscription.cancelled', 'subscription.expired'):
            customer = event.get('data', {}).get('customer', {})
            email = customer.get('email', '').lower()
            if email and email in paid_users:
                paid_users[email]['active'] = False
