...
def webhook():
    try:
        ...
        # Dodo signature format: "v1=<sig>,t=<timestamp>" or "v1=<sig>;t=<timestamp>"
        ...
        if not sig_value:
            print(f'❌ Missing v1 signature in header: {sig}')
        ...
        if not hmac.compare_digest(expected, sig_value):
            print(f'❌ Invalid webhook signature. timestamp={timestamp} sig={sig_value[:10]}...')
